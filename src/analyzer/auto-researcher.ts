...
function buildResearchPrompt(today: string): string {
  return `你是一个技术趋势研究分析师。当前日期是 ${today}。基于提供的开源项目信息，生成一份简洁的深度调研报告。

## 重要约束

- **时间锚定**：当前是 ${today}，你的分析和预测必须基于这个时间点。不要引用已经过时的模型版本或技术（例如不要把 2024 年的模型当作"最新"来讨论）。
- **只基于提供的数据**：你的分析必须基于下方提供的项目信息（README、Issues、Stars、Forks 等实际数据）。不要凭空编造项目特性或功能。如果提供的信息不足以做出某项判断，请明确说明"信息不足"，而非猜测。
- **趋势预测要保守**：预测应基于已有数据中可观察的趋势（star 增长、issue 活跃度、fork 率），而非泛泛的行业猜测。
...
### 趋势预测
一段话：基于该项目当前的增长数据和社区活跃度，预测未来 3-6 个月可能的发展方向。必须基于已观察到的数据趋势，不要引用你训练数据中的过时信息。

请直接输出 Markdown，不要包裹在代码块中。`;
}

export async function generateResearch(input: ResearchInput, today?: string): Promise<ResearchReport> {
  const llm = getLlm();
...
${keywords}`;
  ...
  try {
    const response = await llm.chat.completions.create({
      ...
      max_tokens: 4096,
      messages: [
        { role: 'system', content: buildResearchPrompt(currentDate) },
